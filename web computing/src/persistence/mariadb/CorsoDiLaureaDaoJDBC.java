...
public class CorsoDiLaureaDaoJDBC implements CorsoDiLaureaDao {
	...
	public void save(CorsoDiLaurea corsoDiLaurea) {
		...
		try {
			String insert = "insert into corsodilaurea(nome, dipartimento_codice) values (?,?,?)";
			PreparedStatement statement = connection.prepareStatement(insert);
			statement.setString(1, corsoDiLaurea.getNome());
			statement.setLong(2, corsoDiLaurea.getDipartimento().getCodice());
			...
			String cod = "select max(LAST_INSERT_ID(codice)) from corsodilaurea;";
			statement = connection.prepareStatement(cod);
			ResultSet rSet = statement.executeQuery();
			if(rSet.next())
				corsoDiLaurea.setCodice(rSet.getLong(1));
	...
	private void updateCorsi(CorsoDiLaurea corsodilaurea, Connection connection) throws SQLException {
		...
		for (Corso corso : corsodilaurea.getCorsi()) {
			...
			if(result.next()){
				...
				String iscrivi = "insert into afferisce(corso_codice, corsodilaurea_codice) values (?,?)";
				PreparedStatement statementIscrivi = connection.prepareStatement(iscrivi);
				statementIscrivi.setLong(1, corso.getCodice());
				statementIscrivi.setLong(2, corsodilaurea.getCodice());
