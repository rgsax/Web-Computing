...
public class CorsoDaoJDBC implements CorsoDao {
	...
	public void save(Corso corso) {
		...
		try {
			String insert = "insert into corso(nome) values (?)";
			...
			String cod = "select max(LAST_INSERT_ID(codice)) from corso;";
			statement = connection.prepareStatement(cod);
			
			ResultSet rSet = statement.executeQuery();
			if(rSet.next())
				corso.setCodice(rSet.getLong(1));
	...
	private void updateStudenti(Corso corso, Connection connection) throws SQLException {
		...
		for (Studente studente : corso.getStudenti()) {
			...
			if(result.next()){
				...
				String iscrivi = "insert into iscritto(matricola_studente, corso_codice) values (?,?)";
				PreparedStatement statementIscrivi = connection.prepareStatement(iscrivi);
				statementIscrivi.setString(1, studente.getMatricola());
