...
public class ScuolaDaoJDBC implements ScuolaDao {
	...
	public void save(Scuola scuola) {
		...
		try {					
			String insert = "insert into scuola(codice_meccanografico, nome) values (?,?)";
			PreparedStatement statement = connection.prepareStatement(insert);
			statement.setString(1, scuola.getCodiceMeccanografico());
			statement.setString(2, scuola.getNome());
			...
			String cod = "select max(LAST_INSERT_ID(id)) from scuola;";
			statement = connection.prepareStatement(cod);
			ResultSet rSet = statement.executeQuery();
			if(rSet.next())
				scuola.setId(rSet.getLong(1));
			System.out.println(scuola.getId().toString());
